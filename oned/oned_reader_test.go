...
func TestOneDReader_doDecode(t *testing.T) {
	...
	reader := NewEAN13Reader().(*ean13Reader)
	...
	// reverse with resultpointcallback test via file-based decode
	hints := map[gozxing.DecodeHintType]interface{}{
		gozxing.DecodeHintType_TRY_HARDER: true,
	}
	bmp, _ = gozxing.NewBinaryBitmap(gozxing.NewGlobalHistgramBinarizer(newTestBitSource(1, "")))
	_, _ = reader.doDecode(bmp, hints)
}

func TestOneDReader_Reset(t *testing.T) {
	NewEAN13Reader().Reset() // do nothing
}
